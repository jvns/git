...
First, it makes a list of all commits in the current branch
that are not in `<upstream>`. This is the same set
...
Then the commits that were previously listed are reapplied to the current
branch, one by one, in order. This has a similar effect to `git cherry-pick
<commit>`.
