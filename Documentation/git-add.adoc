...
DESCRIPTION
-----------
Add or remove files to the index to prepare for a commit.

You always have to add changes before committing them. For example, if you've
edited `file.c` and want to commit your changes, you can run:
...
If you're not sure which changes have been added, you can run `git status` to
print out a summary of the changes that will be committed.

`git add` will not add ignored files by default. You can use the `--force`
option to add ignored files. If you explicitly specify the exact filename of an
ignored file (e.g. `git add ignored.txt`), `git add` will fail with a list of
ignored files. Otherwise it will silently ignore the file.
