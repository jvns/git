...
`git checkout` can be used in 5 different ways, called "modes":

1. **Switch branches**, with `git checkout <branch>`
2. **Create a branch**, with `git checkout -b <branch>`
3. **Switch to a commit and detach HEAD**, with `git checkout <commit>`
   or `git checkout --detach <branch>`
4. **Restore a different version of a file**, for example with `git
   checkout <commit> <filename>` or `git checkout <filename>`
5. **Patch mode**

When you run `git checkout <something>`, Git tries to guess whether
`<something>` is intended to be a branch, a commit, or a filename, and
then switches branches, switches commits, or restores the filename.
...
* `git checkout <branch> --` will force Git to treat the parameter as a
  branch name
* `git checkout -- <filename>` will force Git to treat the parameter as
  a filename

Here is a full description of all of the modes:
...
3. **Switch to a commit and detach HEAD**:
`git checkout --detach [<branch>]`::
`git checkout [--detach] <commit>`::
+
This is exactly the same as "switch branches", except that HEAD is set
to a commit ID instead of a branch name. Setting HEAD to a commit ID is
called "detaching HEAD".
+
When the _<commit>_ argument is a branch name, the `--detach` option can
be used to detach `HEAD` at the tip of the branch. Omitting _<branch>_
detaches `HEAD` at the tip of the current branch.
4. **Restore a version of a file from a commit**:
...
5. **Patch mode**:
`git checkout (-p|--patch) [<tree-ish>] [--] [<pathspec>...]`::
	This is similar to the previous mode, but lets you choose which parts
	of a file to restore with an interactive interface. See below for the
...
DETACHED HEAD
-------------
In Git, `HEAD` is a tiny file containing the name of the current branch.
When you commit, Git will update the current branch (for example
`main`) to point at the new commit.

For example, here's what your repository might look like before and
after a commit while on the `main` branch:
...
However, `HEAD` does not always refer to a branch: it can refer to a
commit instead. This is called "detached `HEAD` state".

It's easy to set HEAD to a commit ID instead of a branch
accidentally: running `git checkout` with a tag, remote-tracking branch,
or `HEAD^` will all set HEAD to a commit ID. For example:
...
If you create new commits while in detached `HEAD` state, the commits
you make will not be on any branch: they'll be orphaned. Orphaned
commits can cause problems for 2 reasons:

1. They're difficult to find: you can't run `git checkout <branch>` to
   access those commits.
2. Eventually those commits will be permanently deleted by Git's garbage
   collection.

For example, imagine that we check out the commit `b` in detached HEAD
state, create 2 new commits, and then run `git checkout main`. This is
the result:
...
If you've created commits while in detached HEAD state, here's how
to create a branch that points to those commits so that they're not
orphaned.

If you still have the commit checked out, you can create a new branch at
that commit with:

   git checkout -b mynewbranch

If you have moved away from the commit, then you first have to find its
commit ID. For example, you can use `git reflog` to look at the last 4
commits HEAD pointed to like this:

   git reflog -4 HEAD

Once you have the commit ID, you can check it out and create a new
branch like this:
