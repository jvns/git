...
Git needs to know _where_ to push and _what_ to push.

If you don't specify where to push with the `<repository>` argument,
Git uses the `branch.*.remote` configuration for the current branch
If the configuration is missing, it defaults to 'origin'.

Here are the ways you can specify what to push, in order of priority:

1. Specify it on the command line (for example `git push origin main`)
2. Pass one of the `--all`, `--mirror`, or `--tags` options
3. Set the `remote.*.push` configuration for the current branch
4. Set the `push.default` configuration (See linkgit:git-config[1] for
   the meaning of `push.default`).

If you don't specify what to push, the current branch is pushed to
the corresponding upstream branch. The push is aborted if the upstream
