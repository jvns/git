...
To decide which repository to push to, Git will look at
(in order of priority):

1. The `<repository>` argument (for example `git push dev`)
2. The `branch.*.remote` configuration for the current branch
3. The default is 'origin'

To decide which branches, tags, or other refs to push, Git will look at
(in order of priority):

1. The <refspec> arguments (for example `git push origin main`)
2. The `--all`, `--mirror`, or `--tags` options
3. The `remote.*.push` configuration for the current branch
4. The `push.default` configuration (See linkgit:git-config[1] for
   the meaning of `push.default`).

By default (for example if you run `git push` and none of the above is
configured) the current branch is pushed to the corresponding upstream
branch. The push is aborted if the upstream branch does not have the
...
+
 * `<src>` is often the name of the branch to push, but it can be any
	arbitrary "SHA-1 expression" (see linkgit:gitrevisions[7]).
 * `<dst>` must be the name of a branch, tag, or other ref on the remote.
   It can't be an arbitrary expression. `:<dst>` is optional.
 * `+` is optional. It does the same thing as `--force`
...
`main:other`). Here are the rules for how short refspecs are expanded,
as well as various other special refspec forms:
+
 1. `<src>` without a `:<dst>` means to update the same ref as the `<src>`.
    For example, if `main` is a branch, then the refspec `main` expands
	to `main:refs/heads/main`.
 2. If <dst> unambiguously refers to a ref on the <repository> remote,
    then expand it to that ref.
    For example, if `v1.0` is a tag on the remote, then `HEAD:v1.0`
	expands to `HEAD:refs/tags/v1.0`.
 ...
 7. A refspec starting with ^ is a negative refspec.
	...
	For example, `git push 'refs/heads/*' '^refs/heads/dev-*'`
 ...
 9. Other ambiguity resolutions might be added in the future, but for
...
you're pushing to. Here are the rules. In these rules "update" means any
