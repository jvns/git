...
For example, if your current branch is `main` and you want to update the
`main` branch in the remote `origin` repository, you can run:

  git push origin main

If the `git push` succeeds, the remote branch and the local branch will
be the same.

If you run `git push` without any arguments, git's default behaviour
is to use the `simple` value for `push.default`, which pushes to the
current branch's corresponding upstream branch. As a safety measure, the
push is aborted if the upstream branch does not have the same name as
the local one.

If the remote branch does not already exist, you need to use the `-u`
option to create it.
...
+
If this is not specified, Git consults the `branch.*.remote`
configuration for the current branch to determine where to push. If the
...
+
If this is not specified and the `--all`, `--mirror`, `--tags`
options are not used, the command finds the default `<refspec>` by
consulting `remote.*.push` configuration, and if it is not found,
honors `push.default` configuration to decide what to push (See
linkgit:git-config[1] for the meaning of `push.default`).
+
